    ...
    static func matches(actual: String, expected: String, mode: MatchMode) -> Bool {
        ...
        case .containsAll(let keywords, let maxCER):
            ...
            let keywordsPass = keywords.allSatisfy { keyword in
                ...
                // Try normalized text first (ignores punctuation/whitespace),
                // fall back to raw lowercased text for punctuation keywords.
                return normalizedActual.contains(lowerKeyword)
                    || lowerActual.contains(lowerKeyword)
            }
