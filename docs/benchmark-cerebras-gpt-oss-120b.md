...
|----------|:-------:|:---------:|:-------:|:-------:|:-------:|:---------:|:---:|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | 0.0929 | 27/67 | 35 | 44 | 23 | 98.0s | 0.233x |

> Cloud API 用量：input 24194 tokens, output 15806 tokens, total 40000 tokens
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.033** | 服务器 IP 地址是 192.168.1.100，端口号是 8080。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0** | Kubernetes 的 pod 状态是 CrashLoopBackOff。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.077** | 访问 GitHub .co。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.026** | 软件工程是一门研究用工程化方法构建和维护有效、实用且高质量的软件的学科。它涉及程序设计语言、数据库、软件开发工具、系统平台等方面的知识。现代软件开发通常采用敏捷开发方法，强调快速迭代和持续交付。版本控制系统如 Git 是团队协作开发的基础工具。持续集成和持续部署能够自动化测试和发布流程，提高开发效率。代码审查是保证代码质量的重要实践，团队成员互相审阅代码、修改单元测试和集成测试，帮助开发者在早期发现和修复缺陷。微服务架构将大型应用拆分为多个独立的小型服务，容器化技术如 Docker 简化了应用的部署和运维管理。云计算平台提供弹性的计算资源，支持应用的快速扩展。DevOps 实践将开发和运维紧密结合，促进软件的快速可靠交付。性能优化需要从算法、数据结构、系统架构等多个层面综合考虑。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.154** | 标杆房企必然调整市场占比。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.238** | 那我就暗示 ISBN information systems management。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.351** | 玩 basketball，然后我稍微邀请我的 France 一起打，就是 after cast 的时候。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.100** | 然后刚忘了讲，你是念什么 major？ |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.154** | 数据癌症病成那样，还打着点滴，就更不可能把女主官吊起来。说来说去，彼得认为还有其他人在帮助数据这么做。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.268** | 这位叫 Peter 的 FBI 探员，一上来就进行了一番物理分析，认为阿曼达不可能吊起比他还重的女警官。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.118** | 把这些劳工抓起来，送到月亮去。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.071** | 用 Docker Compose 部署了三个 michao service 到 staging 环境。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.083** | 这个 function 的 return type 应该是 optional 而不是 forced and wrap。 |
