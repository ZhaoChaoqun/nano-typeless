...
|----------|:-------:|:---------:|:-------:|:-------:|:-------:|:---------:|:---:|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | 0.0917 | 29/67 | 35 | 42 | 25 | 140.5s | 0.334x |

> Cloud API 用量：input 23573 tokens, output 16861 tokens, total 40434 tokens
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.160** | MacBook Pro M3 芯片性能提升了 40%。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.118** | Kubernetes 的 pod 状态是 CrashBackOff。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.050** | 在 42 行设置一个 break point。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.263** | 快速语音识别测试 12345。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0** | 人工智能技术在过去 10 年中取得了巨大的进步。深度学习算法使得计算机能够处理和理解自然语言。语音识别技术已经广泛应用于智能手机和智能音箱。自动驾驶汽车使用多种传感器和人工智能算法来感知环境。医疗领域的人工智能可以辅助医生进行疾病诊断。自然语言处理技术让机器能够理解人类的语言并做出回应。计算机视觉技术使得机器能够识别和分析图像中的内容。强化学习技术让人工智能系统能够通过试错来学习最优策略。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.262** | 那我就暗示 ISN Information Systems Management |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.324** | 深圳或者是上海这种城市会有更 opportun。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.122** | 所以， 我的 parents 我的妈妈是 Chemistry 老师，我的爸爸是 History 老师。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.316** | 玩 basketball，然后我稍微邀请我的 france 一起打，就是 after class 的时候。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.130** | 毕业歌会之后，我们还去吃个饭，然后就感。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.212** | 数据异常变成那样，还打着点滴，就更不可能把女主角吊了起来。说来说去，彼得认为还有其他人在帮助数据这么做。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.115** | 当时心里想，我只要能跪，就能站。我在床上练着，跪着走。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.250** | 下车后，望着三十多层的大高楼，发呆。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.351** | 如果你把它当成产品的话，你会觉得消费者需要什么样的？ |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.190** | 媒体也已经报道了，然后债主也已经为楼了。 |
...
| Pipeline | CER | 输出文本 |
|----------|:---:|---------|
| Paraformer + Cloud Rewrite (gpt-oss-120b) | **0.146** | 这个 function 的 return type 应该是 optional 而不是 forced。 |
